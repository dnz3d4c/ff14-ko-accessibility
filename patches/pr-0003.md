...
승인되면 이 순서로 돈다. 되돌릴 수 있는 지점은 fork까지다.

1. `derbruedi/ff14-accessibility`를 fork (공개 저장소가 하나 생긴다)
2. `upstream-habitat-separator`를 fork에 push
3. base `derbruedi/ff14-accessibility:main`, head `dnz3d4c:upstream-habitat-separator`로 PR 생성

fork는 나중에 지울 수 있다(`gh` 토큰에 `delete_repo` 있음). **PR은 열리는 순간 유지관리자에게 알림이 간다.**
