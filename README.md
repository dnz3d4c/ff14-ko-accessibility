...
- **로그와 대화** — 들어오는 줄을 채널별로 읽고 채널마다 따로 끌 수 있습니다. 입력란을 열면 지금 채널을 말하고 치는 글자를 하나씩 읽습니다. 아홉 채널로 나눈 기록을 개수 제한 없이 되읽을 수 있고, 되읽는 중에 Enter를 누르면 그 채널 그대로 답장이 열립니다(말하기·파티·자유부대·귓속말 넷)
- **이동과 길찾기** — 가까운 대상을 키 하나로 넘기며 이름·분류·거리·방향을 읽고 게임 대상으로도 잡습니다. 분류는 스무 개이고 딥 던전 안에서는 다섯으로 바뀝니다. 목표 방향은 신호음으로 나갑니다. 좌우는 목표가 있는 쪽이고, 높낮이는 앞뒤라서 정면일 때 가장 높으며, 크기는 남은 거리라서 가까워질수록 커집니다. 장애물을 피해 걸을 방향을 안내하고 vnavmesh가 있으면 목표까지 자동으로 걸어갑니다. 길 정보에 없는 구간은 한 번 걸어서 기록해 두면 나중에 그 경로를 따라갑니다
- **전투** — 내 HP는 숫자로, 대상 HP는 백분율로 말합니다. HP와 MP는 소리로도 나타나서 가득 차 있으면 오른쪽, 다 떨어졌으면 왼쪽에서 납니다. HP는 높고 둥근 소리, MP는 낮고 유리 같은 소리라 한꺼번에 나도 구별되고, HP가 25퍼센트 아래로 내려가면 HP 소리만 떨립니다. 적이 나를 겨냥해 기술을 쓰면 알리고, 예고된 범위 안에 서 있는 동안 반복해서 소리가 납니다. 전리품 입찰은 아이템 능력치까지 읽어 주고 숫자패드로 고를 수 있습니다
- **소지품과 장비** — 칸마다 이름·레벨·착용 가능 여부를 읽고 못 쓰면 그 이유까지 붙입니다. 아이템 레벨과 방어력과 능력치가 안내에 들어가고, 장비세트에 들어 있는 물건은 팔기 전에 경고합니다
- **단축바** — 어느 키에 어떤 기술이 놓여 있는지 읽습니다. 배운 기술, 소지품의 아이템, 퀘스트 아이템, 일반 기술, 탈것을 키보드만으로 10개 단축바 어디에나 올릴 수 있습니다
- **딥 던전** — 방을 대상으로 고를 수 있습니다. 멀리 떨어져서 게임이 더 이상 다루지 않는 방도 목록에 남아 있어 언제든 거기까지 걸어갈 수 있습니다. 몇 층인지와 층 전체에 걸린 효과·위험도 알려 줍니다. **딥 던전 기능은 아직 게임 안에서 검증되지 않았습니다**
...
### 한국 서버 모드와 원본과 다른 점
공식 파이널 판타지 14 런처를 사용하며, 한국 서버에 맞게 포팅된 달라무드를 사용합니다.
- 모드 메시지가 한국어로 출력됩니다.
- 한국 클라이언트에서 무엇이 어떻게 동작하는지 알려 주는 `/acc compat` 명령이 있습니다(이게 굳이 필요할까?)
- 한국 서버에만 있는 결함이 하나, 원본과 함께 겪는 결함이 둘 있습니다. [사용 안내](overlay/ko/README.ko.md) 6장에 증상과 우회 방법이 있습니다

## 2. 단축키

기능 이름 옆 괄호는 설정 파일에 적는 이름입니다. 단축키를 변경하는 방법과 설정 파일에 고치는 독일어 단축키 이름 표는 [사용 안내](overlay/ko/README.ko.md) 4장에 있습니다.
...
**길안내, 자동 이동, 따라가기는 한 번에 하나만 활성화됩니다.** 그리고 토벌수첩이 열려 있고 마물 줄에 초점이 있으면 `숫자패드 3`이 자동 이동 대신 **그 마물을 찾아 줍니다.**
...
- **Ctrl+F3** — 소지품과 중요 아이템 읽기. 건네주기 창에서는 낼 수 있는 물건만 읽습니다 (`KeyReadInventory`)
- **Shift+F3** — 소지금 말해 주기 (`KeyReadGil`)
- **Ctrl+F4** — 토벌수첩 읽기. 그 창이 열려 있어야 합니다 (`KeyBestiary`)
- **Ctrl+F12** — 들어온 알림·초대 수락하기 (`KeyNotification`)

**`Ctrl+F`는 지금 동작하지 않습니다.** 처음 지정된 키 이름을 모드가 알아보지 못해서입니다. 표에 있는 다른 키로 바꾸면 되살아나고, 그 방법은 [사용 안내](overlay/ko/README.ko.md) 6장에 있습니다.
...
기술 메뉴가 열려 있으면 숫자패드는 메뉴를 다루는 데만 사용됩니다. 숫자패드 8과 2로 목록을 넘기고, 4와 6으로 목록을 바꾸고(기술·아이템·퀘스트 아이템·일반 기술·탈것 다섯), 0으로 고르고, 마침표로 한 단계 뒤로 갑니다.
...
**Page Up**과 **Page Down**은 카메라 확대·축소를, **Ctrl+End**는 카메라 설정 저장을, **숫자패드 5**는 카메라를 대상 쪽으로 돌리는 기능을 겸합니다. 셋 다 화면에만 영향을 주는 기능이라 화면을 보지 않고 플레이할 때는 겹쳐도 아무 일이 일어나지 않습니다.

한국 클라이언트에서는 여기에 **`N`이 하나 더 겹칩니다.** 이것만은 실제로 창이 열립니다.

겹치는 개수는 게임 접속시 음성으로 알려 줍니다.

## 3. 명령어

명령어는 대화 입력창에서 사용할 수 있습니다.
- `/acc help` — 도움말 알려 주기
...
- `/acc fishobj` — 50미터 안의 사물을 훑어 기록에 남기기. 낚시터를 못 찾을 때 쓰는 진단용입니다
...
- `/acc compat` — 한국 클라이언트에서 어느 기능이 어떤 방식으로 동작하는지 말해 주기. **한국 서버판에만 있습니다**(이 기능의 필요성을 재검토 위에도 적음)
- `/acc lang ko` — 안내 언어 바꾸기. `ko`, `en`, `de`, `auto`를 사용할 수 있습니다
- `/acc dump <창 이름>` — 창 이름으로 지정된 창의 구조를 바탕 화면에 저장하기

## 4. 모드 동작을 위한 외부 패키지 목록


- **[원본 접근성 모드](https://github.com/derbruedi/ff14-accessibility)** (AGPL-3.0) — **독일어로 개발되고 글로벌 클라이언트를 전제로 합니다.** 한국 서버용 모드는 한국 서버용 변경을 적용하고, 한국 서버와 무관한 수정 사항은 원본 모드의 동작과 일치합니다.
- **[KR Dalamud 업데이터](https://github.com/MiqoKR/kr-dalamud-updater)** — 한국 서버용으로 포팅된 달라무드 프로그램. 달라무드가 없으면 모드가 동작하지 않습니다.
- **[vnavmesh](https://github.com/awgil/ffxiv_navmesh)** — 자동 이동, 따라가기, 경로 미리 듣기에 필요합니다. 플러그인이 섳치되 지 않아도 자동 이동 관련 기능을 제외한 나머지 기능은 동작합니다.
...
큰 기능 여섯 가지는 [bladestorm360](https://github.com/bladestorm360)이 더했습니다.

- **PR #1** — 적을 넘길 때 레벨과 HP를 함께 말하기, 내 HP를 백분율이 아니라 숫자로 되돌리기
- **PR #2** — 기술 설명에 효과 범위의 모양 넣기
- **PR #3** — 대상 분류 아군과 임무
- **PR #4** — 캐릭터 생성의 외모 단계
- **PR #5** — 게임의 로그 탭과 필터를 따라가는 두 번째 로그 시스템
- **PR #6** — 딥 던전 전체
...
- `log.bat` — 이번에 실행한 결과가 정상인지 로그를 읽어 판정하기
- `check.bat` — 커밋 전 검사. 전체 테스트 → vendor 기록 정합 → KR과 글로벌 양쪽 빌드
- `pack.bat` — 배포 폴더 만들기. 소스를 다시 빌드해서 담고, 담은 것이 KR 빌드가 맞는지 다시 검사합니다
- `sync.bat` — 원본이 얼마나 앞서 갔는지 재고, 우리가 얹힌 자리를 새 버전으로 옮기기
...
배치가 한 가지 일을 한다면, 커맨드는 그 일을 **어떤 순서로 실행하고 어디서 멈출지**를 정합니다.
