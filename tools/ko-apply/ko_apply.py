...
한국어화는 `AccessibilityStrings.cs` 안의 688쌍을 고치는 일이다. 그걸 손편집
diff로 쌓으면 `overlay/patches/0008`이 700줄짜리 패치가 되는데, **그 파일을
업스트림이 계속 고친다** - 8일에 릴리스 7개다(docs/status.md §4-6).
...
`(독일어, 영어) -> 한국어` 표를 갖고, 이 도구가 소스를 다시 쓴다. 패치는
생성물이라 **충돌하면 푸는 게 아니라 버리고 다시 만든다.** 실패 모드가
"diff 충돌"에서 "카탈로그의 문장 N개를 소스에서 못 찾음"으로 바뀐다.

## 무엇을 건드리고 무엇을 안 건드리나

건드리는 자리는 **카탈로그에 있는 쌍뿐이다.** 이게 이중 안전장치다.

- 못 읽는 모양(중첩 삼항 등 손으로 옮길 40곳, docs/ko-hand-cases.md)은 애초에
...
40곳은 이 도구가 못 다룬다. 그건 **별도 패치**로 간다 - 생성 패치에 손편집을
섞으면 다시 만들 때 조용히 사라진다.
