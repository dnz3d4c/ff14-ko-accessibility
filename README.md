...
## 2. 한국 서버 모드가 원본 모드 다른 것

원본은 독일어로 개발되고 글로벌 클라이언트를 전제로 합니다. 한국 서버판이 다른 것은 셋입니다.

- **모드 안내 메시지가 한국어로 출력합니다.** 언어는 `/acc lang`으로 변경할 수 있습니다. 기본값이 한국어입니다
...
배포물에 함께 포함된 제3자 소프트웨어는 **Tolk**(LGPL-3.0), **NVDA Controller Client**(LGPL-2.1), **NAudio**(MIT)입니다. 각각의 라이선스는 `FF14Accessibility.zip` 안의 `THIRD-PARTY-NOTICES.md`에 있고, **다른 사람에게 전달시 라이센스가(이 의미가 맞는지 너가 확인) 함께 있어야 합니다.**
