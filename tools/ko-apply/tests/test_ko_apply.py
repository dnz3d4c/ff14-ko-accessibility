...
def test_별칭으로_갈라지는_자리는_못_본다():
    # **못 보는 것을 안 보인다고 적어 두는 자리다.** 이 파서의 표식은
    # `IsGerman`과 `Pick`뿐이라, 별칭을 따로 정의하고 그 별칭으로 갈라지는
    # 자리는 한 건도 안 잡힌다. `ColorNamer.cs:32`가 `De => Loc.IsGerman`을
    # 두고 색 묘사 104곳이 거기로 갈라진다(2026-08-19 기준) - 전부 한국어
    # 가지가 없어 영어로 발화되는데 이 검사는 통과한다.
    #
    # 그러니 **0건은 "다 옮겼다"가 아니다.** 세는 쪽은 `tools/strings-golden`의
    # `scan_aliases`이고, 옮기는 것은 현황판 `W-44`다. 이 테스트가 초록인 채로
    # 남아 있는 동안은 사각지대가 그대로라는 뜻이다.
...
def test_한국어_없이_영어로_나가는_자리가_없다():
    ...
    #
    # **초록이 "다 옮겼다"는 뜻은 아니다.** 표식 `IsGerman`·`Pick`으로 갈라지는
    # 자리만 본다 - 별칭 104곳은 위 `test_별칭으로_갈라지는_자리는_못_본다`가
    # 적어 둔 사각지대다.
