...
**같은 것을 배포물에도 적용한다.** 설치 프로그램이 대신할 수 있는 일을 안내 문장으로 넘기지 않는다. 남의 물건이라서 못 한다는 판단이 나오면 **정확히 무엇이 막는지**까지 확인한다 — 라이선스가 없으면 막히는 것은 *재배포*지 *사용자 대신 받아 오기*가 아니다. 실제로 KR 달라무드 업데이터를 그 구분 없이 사용자에게 떠넘기고 있었고, 같은 실행 안에서 vnavmesh는 받아 주고 있었다(2026-08-19, W-33). **한쪽은 해 주고 한쪽은 안 해 주고 있으면 그 자체가 신호다.**
