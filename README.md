...
기능 이름 옆 괄호는 설정 파일에 적는 이름입니다. 키를 바꾸는 방법과 설정 파일에 쓰는 독일어 키 이름 표는 [사용 안내](overlay/ko/README.ko.md) 4장에 있습니다.
...
**길안내, 자동 이동, 따라가기는 한 번에 하나만 켜집니다.** 하나를 켜면 나머지가 꺼집니다. 그리고 토벌수첩이 열려 있고 마물 줄에 초점이 있으면 `숫자패드 3`이 자동 이동 대신 **그 마물을 찾아 줍니다.**
...
기술 메뉴가 열려 있는 동안 숫자패드는 메뉴를 다루는 데만 쓰입니다. 숫자패드 8과 2로 목록을 넘기고, 4와 6으로 목록을 바꾸고(기술·아이템·퀘스트 아이템·일반 기술·탈것 다섯), 0으로 고르고, 마침표로 한 단계 뒤로 갑니다.
...
**Page Up**과 **Page Down**은 카메라 확대·축소를, **Ctrl+End**는 카메라 설정 저장을, **숫자패드 5**는 카메라를 대상 쪽으로 돌리는 기능을 겸합니다. 셋 다 화면에만 영향을 주는 기능이라 화면을 보지 않고 플레이할 때는 겹쳐도 아무 일이 일어나지 않고, 일부러 그렇게 두었습니다. 숫자패드 5만은 모드가 키를 받고 게임에 넘기지 않아서 카메라가 같이 움직이지 않습니다.
...
이 모드는 다른 프로그램 넷 위에서 동작하고 **넷 다 다른 사람이 만든 것입니다.** 우리가 하는 일은 맨 위에 있는 접근성 모드를 한국 서버에 맞추는 것입니다.
