...
KR 전용 결함과 "게임과 답이 다른 두 곳"은 정리했고, 설치기를 KR용으로 돌려 **폴더에 손으로 넣고 빼는 절차를 없앴다.** 상류로 돌려보낼 수 있는 수정도 하나 나왔다(타이틀 메뉴 항목 수).

**남은 일은 전부 [프로젝트 현황판](docs/status.md)에 있다.** 뭘 고치기 전에 거기를 먼저 연다.

- **[프로젝트 현황판](docs/status.md)** — 남은 일의 단일 원천
...
- `patches/` — **업스트림에 올릴** 변경. 오버레이보다 **먼저** 적용된다
