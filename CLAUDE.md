...
기능 구현·수정·개선을 **시작하기 전에** [docs/status.md](docs/status.md)를 연다. 남은 일의 단일 원천이고, 다른 문서는 근거만 갖는다.

- 하려는 일이 판에 없으면 `W-NN`을 새로 따고 시작한다. ID는 재사용하지 않는다.
- 끝나면 판의 상태를 옮긴다. **완료 선언 전에 한다** — 나중에 몰아서 하지 않는다.
- `[상류]`·`[오버레이]`·`[검증]`·`[도구]` 커밋은 판을 같이 건드리거나 `Status-Board:` 트레일러로 이유를 밝혀야 한다. 훅의 C8이 검사한다.
