...
핀 이전(v5.84까지)의 이력은 여기 없다. 우리가 그 위에서 시작했을 뿐 따라온 것이 아니라서다. 절차는 [sync.md](sync.md).
...
우리 쪽: 패치 **0001~0007은 그대로 붙었고**, 한국어 생성 패치(`0008`)는 붙이지 않고 카탈로그에서 **다시 만들었다**([sync.md](sync.md) §5.4). 카탈로그 669개가 소스에서 전부 제자리를 찾았다 — 업스트림이 우리가 옮긴 문장을 고친 것은 없다.
