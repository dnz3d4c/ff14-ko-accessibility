...
1. **`AtkResNode.IsVisible()` 시그니처가 KR에서 해석되지 않는다** — 조사 §6-12가 예측한 항목이 그대로 나왔다. 60개 호출부가 전부 예외. 타이틀 메뉴 이동·창 전체 읽기·노드 덤프가 죽었다. `overlay/patches/0002`로 수정
...
- **기어세트 마크**가 아이템 인스턴스가 아니라 id 단위로 판정된다(`overlay/patches/0001`). 경고가 남는 쪽이라 안전한 방향
- **노드 가시성**이 게임 함수가 아니라 플래그+부모 사슬로 판정된다(`overlay/patches/0002`). 부모 사슬 부분은 추론이지 검증된 게 아니다

둘 다 단계 5 검증기가 기동 시 1회 알리게 해야 한다. **지금은 조용하다 — 이 프로젝트에서 침묵은 허용되지 않는 실패 모드다.**
