...
**전에는 우리가 세 군데에 박아 뒀다** — 설치 프로그램·`run/_env.cmd`·`tools/kr-setup/check_log.py`. 셋이 서로 맞는지도, 업데이터의 실제 설정과 맞는지도 아무도 안 봤다. 사용자가 값을 옮기면 설치 프로그램은 옛 폴더에 넣고 업데이터는 **빈 프로필을 새로 만들어 거기 주입한다** — 오류 없이 플러그인만 빠진다. [status.md](../status.md) §6-1이 "아무 일도 안 일어난다의 원인은 늘 경로 중 하나다"라고 적어 둔 바로 그 실패다.
...
없거나 깨졌거나 못 쓸 값이면 조용히 3번으로 간다. **남의 파일이 깨졌다고 우리 설치 프로그램이 죽지 않는다.**

**"남의 것은 남이 관리하게 둔다"(현황판 §5-7)에 어긋나지 않는다.** 그 방침이 막는 것은 남의 설정 파일을 우리가 **쓰는** 것이고(vnavmesh 설정을 우리가 만들어 주면 그쪽 스키마에 묶인다), 여기서 하는 것은 **읽기**다. 그리고 박아 두는 쪽이 결합이 더 세다 — 박으면 "그쪽 기본값이 안 바뀐다"와 "사용자가 안 고친다" 둘 다에 걸어야 하는데, 읽으면 앞의 하나만 남고 그마저 폴백이 받는다.
...
| 6 | vnavmesh 동작 | **해소** | 인게임에서 자동 이동 전 구간 확인 — 적재·경로 탐색 9회 전부 성공 (2026-08-18, [status.md](../status.md) §7). §7 |
...
| 11 | 커스텀 저장소 로드 | **해당 없음** | 설치 프로그램이 파일을 직접 놓는다. 저장소를 등록하지 않아도 정식 플러그인으로 뜬다 |
...
|------|------|--------------|
| 게임 함수 (글로벌) | 매번 남는다 | 없음 |
| 게임 함수 + KR 시그니처 | 매번 남는다 | 없음 — 답이 같다 |
| 모드 내 구현 (가시성) | 경고로 남는다 | **1회** |
| id 단위 (기어세트) | 경고로 남는다 | **1회** |
...
Git Bash에서 `/acc keys` 같은 인자를 넘기면 MSYS 경로 변환이 `C:/Users/.../acc keys`로 바꿔 버린다. 넘길 때는 `MSYS_NO_PATHCONV=1`을 붙인다.
...
**재확인 명령**(게임 패치 후 이걸 먼저 돌린다. 이 검사는 테스트에도 들어 있다 — `tools/sig-probe/tests/test_shipped_signature.py`가 빌드가 실제로 컴파일하는 vendor 소스에서 시그니처를 꺼내 대조한다):

uv run --no-project --with pytest pytest tools/sig-probe/tests -q

## 7. vnavmesh 사전 검증 (2026-08-18)

게임을 켜지 않고 할 수 있는 검증은 전부 통과했고, **인게임 동작도 확인됐다** — 대상 지정부터 도착 안내까지, vnavmesh 적재·경로 탐색 9회 전부 성공(2026-08-18, [status.md](../status.md) §7). 게임 패치 뒤에 다시 판정하는 방법은 `docs/dev/kr-runtime.md` §10 — 키를 사람이 눌러야 하고 이 클라이언트에는 키 주입이 통하지 않는다(§5).
...
| 무엇 | 값 | 어디서 |
|------|-----|--------|
| 기호 발음 수준 | `symbolLevel = 0` — **기호를 하나도 발음하지 않는다** | `%APPDATA%\nvda\nvda.ini:20` |
| 백틝 `` ` `` | `그레이브`, 레벨 `all` | `C:\Program Files\NVDA\locale\ko\symbols.dic:79` |
| 대괄호 `[` `]` | `여는/닫는 대괄호`, 레벨 `all` | 같은 파일 74~75줄 |
| Phonetic Punctuation 애드온 | 설치·활성 (`enabled = True`) | `%APPDATA%\nvda\earconsAndSpeechRules.json` |

**함의가 셋이다.**

- **백틱과 대괄호는 지금 소리가 완전히 같다.** 둘 다 레벨이 `all`이라 수준을 "대부분"까지 올려도 안 읽히고, "모두"로 올리면 **둘 다** 읽힌다. 어느 수준에서도 한쪽만 들리는 상태가 없다
- **볼드의 별표도 안 읽힌다.** `symbolLevel = 0`이므로 원문 낭독에 기호 부담이 없다. 볼드를 줄인 이유는 낭독 부담이 아니라 **강조가 199곳이면 강조가 아니기 때문**이다
- **소리가 나는 유일한 기호는 괄호다.** PP 애드온의 `LeftParen`/`RightParen` 규칙이 켜져 있고 대괄호 규칙은 꺼져 있다. 문서의 괄호 90개는 대부분 키 목록의 설정 이름(`(KeyNextObject)`)이라, **정작 눌러야 하는 대상은 조용하고 참고용 이름만 소리가 난다**

**그래도 버튼을 대괄호로 적는다.** 되살리는 비용이 다르기 때문이다 — 대괄호는 PP에 규칙과 소리 파일(`punctuation\LeftBracket.wav`)이 이미 있고 스위치만 꺼져 있어서 켜면 즉시 소리가 붙는다. 백틱은 규칙 자체가 없어 새로 만들어야 하고 사전 레벨도 `all`이다.

`HEADING1`~`HEADING6`과 `BOLD` 규칙은 켜져 있지만 **서식이 있는 문서에서만** 동작한다. 마크다운 원문을 그대로 읽으면 `#`은 그냥 문자다.
