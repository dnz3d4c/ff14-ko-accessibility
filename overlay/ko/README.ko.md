...
- `이 프로그램은 우리가 만든 것이 아니고 재배포하지도 않습니다 - 원 저장소에서 직접 받습니다.`
...
- 진행 상황 영역에 `한국 서버용 Dalamud가 설치되지 않았습니다.`가 남고, 이어서 **다음에 무엇을 할지 알려 주는 줄이 셋 더** 남습니다. 업데이터를 실행해 [업데이트 확인]을 누르라는 안내, 업데이터의 경로, 끝나면 설치 프로그램을 다시 실행하라는 안내입니다
- **`== 결과 ==` 블록이 나오지 않습니다.** 다음 절의 성공 표지도, `실패:`로 시작하는 줄도 없습니다
- 그런데도 `끝났습니다. 자세한 내용은 창의 진행 상황 영역에서 확인할 수 있습니다.`라는 대화 상자가 뜹니다

#### ⑦ Dalamud 받기

**게임이 꺼져 있는 상태에서** KR Dalamud 업데이터 창에서 [업데이트 확인] 버튼을 누릅니다. Tab으로 버튼을 훑어 찾을 수 있습니다.
...
- `이 플러그인은 우리가 만든 것이 아니고 재배포하지도 않습니다 - 원 저장소에서 직접 받습니다.`
...
※ 설치 프로그램이 `DALAMUD_RUNTIME을 설정했습니다`와 `이 값은 게임을 새로 켜야 반영됩니다`를 말한 경우, 게임이 켜져 있었다면 한 번 껐다 켜야 합니다.
