...
> **이 문서는 2026-08-17에 동결됐다.** 착수 전 조사이고 그때의 값과 계획을 그대로 둔다 — 지금 값으로 맞추지 않는다. **지금 무엇이 남았는지는 [현황판](../status.md) §2가, 지금 절차는 [environment.md](../dev/environment.md)와 [kr-runtime.md](../dev/kr-runtime.md)가 갖는다.** 여기 적힌 단계와 숫자를 현재 절차로 쓰지 않는다(§13이 그 차이를 짚는다).
