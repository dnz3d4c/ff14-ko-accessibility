...
고친다** - 8일에 릴리스 7개다(overlay/patches/README.md).
...
**같은 파서라 사각지대도 그대로 물려받는다.** 별칭으로 갈라지는 104곳은 안
잡히므로 "0건"을 "다 옮겼다"로 읽으면 안 된다 - 아래 "이 검사가 못 보는 것".
...
# ## 이 검사가 못 보는 것
#
# 파서가 보는 표식은 `IsGerman`과 `Pick` 둘뿐이다. **별칭을 따로 정의하고 그
# 별칭으로 갈라지는 자리는 아예 안 센다.** `ColorNamer.cs:32`가
# `private static bool De => Loc.IsGerman;`을 두고, 그 `De`로 갈라지는 색 묘사가
# 2026-08-19 기준 104곳이다 - 전부 한국어 가지가 없어 영어로 발화되는데 여기서는
# 한 건도 안 잡힌다.
#
# **그러니 "0건"은 "다 옮겼다"가 아니다.** 세는 쪽은 `tools/strings-golden`의
# `scan_aliases`이고, 옮기는 것은 현황판 `W-44`다. 범위를 여기서 넓히지 않는
# 이유는 골든이 691에서 795로 뛰는 큰 변경이라 그 작업으로 따로 열려 있어서다.
...
def missing_korean(root: Path = SOURCE_ROOT) -> list[str]:
    ...
    **다만 0건은 "다 옮겼다"가 아니다.** 표식 `IsGerman`과 `Pick`으로 갈라지는
    자리만 본다 - 별칭으로 갈라지는 104곳(`ColorNamer.cs:32`의 `De`)은 여기
    안 잡힌다. 위 절 "이 검사가 못 보는 것"에 왜인지 적혀 있다.
    """
    problems: list[str] = []
    for path in source_files(root):
        name = path.relative_to(root).as_posix()
...
def main(argv: list[str]) -> int:
    ...
    if "--write" not in argv:
        ...
        # 통과 문구가 "다 옮겼다"로 읽히면 이 검사가 있는 이유가 사라진다.
        # 무엇을 보고 무엇을 안 보는지 여기서 밝힌다. 화면에 나가는 줄이라
        # 마크다운 장식은 안 쓴다 - 스크린리더가 그대로 읽는다.
        print("통과 - 소스가 카탈로그대로고, 표식으로 갈라지는 자리에는 "
              "한국어가 빠진 곳이 없다")
        print("  다만 이 검사는 IsGerman과 Pick만 본다. 별칭으로 갈라지는 "
              "자리는 안 센다 -")
        print("  ColorNamer.cs:32의 De 104곳이 그렇고, 전부 영어로 발화된다. "
              "세는 쪽은")
        print("  tools/strings-golden의 scan_aliases, 옮기는 것은 현황판 W-44다")
        return 0
