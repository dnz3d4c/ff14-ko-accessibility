...
우리 쪽: 패치 **0001~0007은 그대로 붙었고**, 한국어 생성 패치(`0008`)는 붙이지 않고 카탈로그에서 **다시 만들었다**([upstream-sync.md](upstream-sync.md) §5.4). 카탈로그 669개가 소스에서 전부 제자리를 찾았다 — 업스트림이 우리가 옮긴 문장을 고친 것은 없다.
